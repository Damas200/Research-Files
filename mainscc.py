...
print("b1_index", b1_index.shape)
print("b1_val", b1_val.shape)
...
print('tri_idx', tri_indices.shape)
...
for s in seeds:
    ...
    #take 80% of y 
    train_idx = np.random.permutation(len(y))[:int(0.7*len(y))]
    print(train_idx)
    ...
    test_idx = np.random.permutation(testval_idx)[:int(0.667*len(testval_idx))]
...
#BEST PARAMS FOR EDGE OVER ISUFI 'dropout': 0.49535205074569727, 'alpha_leaky_relu': 0.07212654314370462, 'k': 3, 'hid_dim': 65
    ...
    # # # train model
    #eary stopping
    trainer = pl.Trainer(max_epochs=500, accelerator='cpu', devices=1) #callbacks = [early_stop_callback],deterministic=True,benchmark=True)
